...
public:
    ...
    iterator Emplace( const_iterator pos, Args&&... args )
    {
        ...
        if ( size_ == Capacity() )
        {
            ...
            size_t pos_num = std::distance( cbegin(), pos );
            ...
            try
            {
                if constexpr ( std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T> )
                {
                    std::uninitialized_move( position, end(), new_data.GetAddress() + pos_num + 1 );
                }
                else
                {
                    std::uninitialized_copy( position, end(), new_data.GetAddress() + pos_num + 1 );
                }
            }
            catch ( ... )
            {
        ...
        else
        {
            ...
            else
            {
                ...
                std::move_backward( position, std::prev( end() ), end() ); // can throw exception???
                *position = std::move( tmp_value );
            }
        }
        ++size_;
        return position;
    ...
    T& EmplaceBack( Args&&... args )
    {
        ...
        {
            RawMemory<T> new_data( size_ == 0 ? 1 : size_ * 2 );
            auto addr = new_data.GetAddress() + size_;
        ...
        {
            auto addr = data_.GetAddress() + size_;
            new ( addr ) T( std::forward<Args>( args )... );
        }
